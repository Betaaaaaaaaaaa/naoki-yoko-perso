    ...
    async execute(client, message, args) {
        ...
        if (owner.get(`owners.${message.author.id}`) || (config.app.owners.includes(message.author.id) || config.app.funny.includes(message.author.id)) === true) {
            ...
            if (args[0] == 'add') {
                ...
                if (db.get(`${message.guild.id}.antivoc.${newChannel.id}`) === newChannel) return message.channel.send(`<:voc:959943683413782568>・__Le salon <#${db.get(`${message.guild.id}.antivoc.${newChannel}`)}> est désormais interdit__\n*(Bypass par les owners et la Whitelist)*`)
                else {
                    db.set(`${message.guild.id}.antivoc.${newChannel.id}`, newChannel.id)
                    message.channel.send(`<:voc:959943683413782568>・__Le salon ${newChannel} est désormais interdit__\n*(Bypass par les owners et la Whitelist)*`)
